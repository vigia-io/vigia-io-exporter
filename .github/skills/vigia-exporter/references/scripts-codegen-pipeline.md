# Pipeline de codegen scripts.go

## Fluxo

```mermaid
flowchart LR
    A[vigia-io-scripts/providers/] --> B[codegen CLI]
    B --> C[vigia-io-exporter/internal/providers/*/scripts.go]
    C --> D[go build]
```

## Entrada: manifest YAML

```yaml
# vigia-io-scripts/providers/sqlserver/sessions/blocking/manifest.yaml
id: sqlserver.sessions.blocking
provider: sqlserver
category: sessions
name: Blocking Chain
sql_file: blocking.sql
severity_default: critical
tier: open
```

## Comando codegen

```bash
cd vigia-io-scripts
go run ./cmd/codegen \
  --provider sqlserver \
  --output ../vigia-io-exporter/internal/providers/sqlserver/scripts.go
```

## Template de saída

```go
// Code generated by vigia-io-scripts v0.1.0. DO NOT EDIT.

package sqlserver

// Scripts contém queries de monitoramento para SQL Server.
var Scripts = map[string]string{
    "sqlserver.sessions.blocking": `SELECT ...`,
}
```

## CI

1. PR em `vigia-io-scripts` roda testes SQL em containers
2. Artefato `scripts.bundle.json` publicado no registry privado
3. `vigia-io-exporter` CI baixa bundle e roda codegen antes do build
4. Check `git diff --exit-code` garante sincronização

## Legado DbaMonitor

O antigo fluxo usava tabela `ScriptOffline` em SQL Server + `create-script/main.go`.
Substituir por manifests YAML versionados em Git — **não** depender de DB central.

Migração:

1. Exportar `ScriptOffline` para YAML
2. Deprecar `cmd/deploy/create-script`
3. Remover dependência de `DbaMonitor` database
